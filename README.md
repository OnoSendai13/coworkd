...
**Forked from:** This repo is designed to complement a [Hermes Agent](https://github.com/anthropics/hermes-agent) fork with integrated `cowork_tools.py`. Coworkd is the **daemon side**; Hermes with `cowork_tools.py` is the **tool interface side**. Communication between the two is via a shared `context.json` file and Redis pub/sub.
